...
bool overall_schedule::schedule_cars_one_time_unit(int car_priority) {
    ...
    while (this->cars_running_wait_state_n > 0) {
        ...
        for (map<int, cross>::iterator iter = this->crosses.begin(); iter != this->crosses.end(); iter ++) {
            wait_to_termination_n += iter->second.schedule_cars_in_cross(this->cars_running_n, this->cars_arrive_destination_n, this->all_cars_running_time, this->T, this->arrive_car_id_count, this->arrive_T, this->cars_wait_schedule_start_time_n, this->roads_connect_cross, this->cars, this->epoch_start_time);
        }
...
//    this->initial_cars_state_in_T0();
    ...
    while (!whether_vector_zero(this->cars_wait_schedule_start_time_n) || !whether_vector_zero(this->cars_running_n)) {
        ...
        for (list<road>::iterator iter = this->roads_connect_cross.begin(); iter != this->roads_connect_cross.end(); iter ++)
            iter->count_real_situation_car_running_in_road(this->T - this->epoch_start_time);
    }
//    this->output_schedule_status();
    ...
    while (true) {
        ...
        while (true) {
            ...
            for (map<int, road*>::iterator road_iter = roads_departure_cross.begin(); road_iter != roads_departure_cross.end(); road_iter ++) {
                ...
                int through_time = road_iter->second->get_through_time(updata_cross_time, car_iter.get_priority(), turn_direct, car_iter.get_speed(), this->cars, this->epoch_start_time);
                int next_cross_id = (road_iter->second)->get_to();
                double congestion = road_iter->second->check_capacity(updata_cross_time - this->epoch_start_time, through_time - this->epoch_start_time - 1, car_iter.get_speed());
                if (congestion < 1) {
        ...
        while (cross_id != car_iter.get_from()) {
            ...
            roads_departure_cross[road_id]->car_running_count(pre_cross_arrive_time - this->epoch_start_time, next_cross_arrive_time - this->epoch_start_time - 1, this->priority_weight[car_iter.get_priority()]);
...
void overall_schedule::cars_respect_path_regulation() {
    ...
    for (int car_priority = this->my_config.priority_N - 1; car_priority >= 0; car_priority --) {
        ...
        for (list<road>::iterator iter = this->roads_connect_cross.begin(); iter != this->roads_connect_cross.end(); iter ++)
            iter->count_situation_car_wait_into_road(0, finish_T - this->epoch_start_time);
        int start_time = 1;
        int schedule_step = 1;
        ...
        while (true) {
            ...
            this->cars_path_regulation(start_time, schedule_step, car_priority);
...
// path regulation priority car and normal car together, but priority first attempt start run
void overall_schedule::cars_together_path_regulation() {
    // init situation car running in_road
    for (list<road>::iterator iter = this->roads_connect_cross.begin(); iter != this->roads_connect_cross.end(); iter ++) {
        iter->init_situation_car_running_in_road();
    }
    ...
    while (true) {
        ...
        this->cars_path_regulation(start_time, schedule_step, 0);
        start_time += schedule_step;
